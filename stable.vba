...
Application.Calculation = xlManual
    ...
    'Worksheet shortcuts
    '-------------------
    Set StartPond = Application.Worksheets("SP0A")
    ...
    '--------------------------------
    'BrineDict_c matches item = column # with key = Brine species name in Chemstry 2020 worksheet
    ...
    'DeadSeaMon_r matches item = row # with key = month name in Chemstry 2020 worksheet
    ...
    'ReturnMon_r matches item = row # with key = month name in Chemstry 2020 worksheet
    ...
    'solidsDict_c matches item = column # with key = solid species name in SP0A worksheet
    ...
    'HarvDict_c matches item = column # with key = solid species name in SP0A worksheet
    ...
    'liquidsColl_c matches item = column # with key = liquid species name in SP0A worksheet
    ...
    For mc_run = 1 To mcruns
        ...
        MC.Range("evaprandCF").Copy
        Lookup.Cells(10, 35).PasteSpecial Paste:=xlPasteValues, Transpose:=True
        ...
        For step = 1 To totsteps
            ...
            Do While ContinueRun
                ...
                For Each Row In InputOLI.Range("oliinput")
                    ...
                    Select Case CurrentWS
                        Case "SP0A", "C4"
                            If CurrentWS = "SP0A" And BrineDict_c.Exists(species) Then
                ...
                '---------------------------------------
                With OutputOLI
                    V_lnij = .Range("liqvolume").Value
                    V_tnij = .Range("totvolume").Value
                    ...
                    TotalMass = .Range("totalmass").Value                                                    'total mass for TDS calculation
                ...
                'calculate TDS
                If V_lnij <> 0 Then
                    TDS = (TotalMass - WaterMass) / V_lnij
                ...
                If ((V_tnij - Q) < MaxVol) Then
                    DischargeVol = MinDisch
                Else
                    DischargeVol = V_tnij - Q - MaxVol
...
End Sub
Sub elementAll(HarvDict_c As Scripting.Dictionary, liquidsColl_c As Collection, species_r As Collection, totsteps)
    ...
    Dim TimeStep As Integer
    Dim liq_start As Integer
    Dim liq_end As Integer
    Dim brine_start As Integer
    Dim harv_start As Integer
    Dim harv_end As Integer
    Dim harvOut_start As Integer
    Dim step As Integer
    Dim col As Integer
    
    Dim TotalMass As Double
    Dim HarvestRatio As Double
    Dim species_moles As Double
    Dim species_mw As Double
    Dim Mg_sum As Double
    Dim Ca_sum As Double
    Dim K_sum As Double
    Dim Na_sum As Double
    Dim Cl_sum As Double
    Dim H2O_sum As Double
    Dim Mg_frac As Double
    Dim Ca_frac As Double
    Dim K_frac As Double
    Dim Na_frac As Double
    Dim Cl_frac As Double
    Dim H2O_frac As Double
    
    Set inputpond = Sheets(Range("wsname").Value)
    Set Lookup = Application.Worksheets("Look Up Tables")
    
    base_row = inputpond.Range("liqrange").Row
    TimeStep = base_row + 1
    
    liq_start = inputpond.Range("liqrange").Column
    liq_end = inputpond.Range("liqrange").Columns(inputpond.Range("liqrange").Columns.Count).Column
    
    brine_start = inputpond.Range("elembrine").Column
    
    harv_start = inputpond.Range("harvrange").Column
    harv_end = inputpond.Range("harvrange").Columns(inputpond.Range("harvrange").Columns.Count).Column
    harvOut_start = inputpond.Range("elemharv").Column
    
    'brine calculations
    For step = 1 To totsteps
        TotalMass = 0: H2O_sum = 0: Mg_sum = 0: Ca_sum = 0: K_sum = 0: Na_sum = 0: Cl_sum = 0
        For col = liq_start To liq_end
            species = inputpond.Cells(base_row, col).Value
            species_moles = inputpond.Cells(TimeStep + 1, liquidsColl_c(species)).Value
            species_mw = Lookup.Cells(species_r(species), 3).Value
            Mg_sum = Mg_sum + species_moles * Lookup.Cells(species_r(species), 4).Value
            Ca_sum = Ca_sum + species_moles * Lookup.Cells(species_r(species), 5).Value
            K_sum = K_sum + species_moles * Lookup.Cells(species_r(species), 6).Value
            Na_sum = Na_sum + species_moles * Lookup.Cells(species_r(species), 7).Value
            Cl_sum = Cl_sum + species_moles * Lookup.Cells(species_r(species), 8).Value
            H2O_sum = H2O_sum + species_moles * Lookup.Cells(species_r(species), 9).Value
            TotalMass = TotalMass + (species_moles * species_mw)
        Next
        
        Mg_frac = Mg_sum * 24.305 / TotalMass
        Ca_frac = Ca_sum * 40.078 / TotalMass
        K_frac = K_sum * 39.098 / TotalMass
        Na_frac = Na_sum * 22.99 / TotalMass
        Cl_frac = Cl_sum * 35.45 / TotalMass
        H2O_frac = H2O_sum * 18.02 / TotalMass
        inputpond.Cells(TimeStep, brine_start).Value = Mg_frac
        inputpond.Cells(TimeStep, brine_start + 1).Value = Ca_frac
        inputpond.Cells(TimeStep, brine_start + 2).Value = K_frac
        inputpond.Cells(TimeStep, brine_start + 3).Value = Na_frac
        inputpond.Cells(TimeStep, brine_start + 4).Value = Cl_frac
        inputpond.Cells(TimeStep, brine_start + 5).Value = H2O_frac
        TimeStep = TimeStep + 1
    Next step
    
    'harvested salts calculations
    TimeStep = base_row + 1
    For step = 1 To totsteps
        TotalMass = 0: H2O_sum = 0: Mg_sum = 0: Ca_sum = 0: K_sum = 0: Na_sum = 0: Cl_sum = 0
        HarvestRatio = inputpond.Cells(TimeStep, inputpond.Range("harvest").Column).Value
        If HarvestRatio <> 0 Then
            For col = harv_start To harv_end
                species = inputpond.Cells(base_row, col).Value
                species_moles = inputpond.Cells(TimeStep, HarvDict_c(species)).Value
                species_mw = Lookup.Cells(species_r(species), 3).Value
                Mg_sum = Mg_sum + species_moles * Lookup.Cells(species_r(species), 4).Value
                Ca_sum = Ca_sum + species_moles * Lookup.Cells(species_r(species), 5).Value
                K_sum = K_sum + species_moles * Lookup.Cells(species_r(species), 6).Value
            ...
            If TotalMass <> 0 Then
                ...
                H2O_frac = H2O_sum * 18.02 / TotalMass
            Else
                Mg_frac = 0: Ca_frac = 0: K_frac = 0: Na_frac = 0: Cl_frac = 0: H2O_frac = 0
            End If
        End If
        inputpond.Cells(TimeStep, harvOut_start).Value = Mg_frac
        inputpond.Cells(TimeStep, harvOut_start + 1).Value = Ca_frac
        inputpond.Cells(TimeStep, harvOut_start + 2).Value = K_frac
        inputpond.Cells(TimeStep, harvOut_start + 3).Value = Na_frac
        inputpond.Cells(TimeStep, harvOut_start + 4).Value = Cl_frac
        inputpond.Cells(TimeStep, harvOut_start + 5).Value = H2O_frac
        TimeStep = TimeStep + 1
    Next step
